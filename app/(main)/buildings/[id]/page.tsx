...
const deriveRoomStats = (rooms: Room[], fallbackAverage: number) => {
  ...
  const averagePrice =
    prices.reduce((sum, price) => sum + price, 0) / rooms.length;
  const floors = new Set(rooms.map((room) => room.floor)).size;
  const totalCapacity = rooms.reduce(
    (sum, room) => sum + (parseInt(room.capacity || "0", 10) || 0),
    0,
  );
...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      description:
        building.description || `Thông tin chi tiết về ${building.name}`,
...
export default async function BuildingDetailPage({
  ...
  const insightCards = [
    ...
    },
    {
      ...
      value: roomStats.totalCapacity
        ? `${roomStats.totalCapacity}+ người`
        : `${rooms.length} phòng`,
              ...
              <div className="lg:col-span-3 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-base text-white/80 leading-relaxed max-w-3xl">
                    {building.description ||
                      "Không có mô tả cho tòa nhà này. Chúng tôi sẽ cập nhật thêm thông tin sớm nhất để bạn có góc nhìn đầy đủ hơn."}
                  </p>
                </div>
                  ...
                  <div className="flex flex-col gap-4">
                    <div>
                      ...
                      <p className="mt-3 text-gray-600 dark:text-gray-300 leading-relaxed">
                        {building.description ||
                          "Tòa nhà hiện chưa có mô tả chi tiết. Tuy nhiên, toàn bộ phòng đều được vận hành theo tiêu chuẩn an toàn, tiện nghi và sẵn sàng bàn giao ngay."}
                      </p>
                    </div>
                  ...
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <span className="text-sm text-gray-500 dark:text-gray-400">
                      {amenityHighlights.length
                        ? `Tổng hợp từ ${amenityHighlights.length} tiện ích phổ biến`
                        : "Đang cập nhật"}
                    </span>
                  </div>
                  {amenityHighlights.length ? (
                    ...
                    <p className="text-gray-600 dark:text-gray-400">
                      Chưa có dữ liệu tiện ích. Các phòng vẫn được trang bị đầy
                      đủ tiện nghi cơ bản.
                    </p>
                  )}
                </div>
                ...
                <div className="rounded-3xl bg-white dark:bg-gray-900 shadow-xl ring-1 ring-gray-100/70 dark:ring-gray-800/70 p-6 sm:p-8">
                  ...
                  {contactInfo ? (
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400 mt-3">
                      Chưa cập nhật thông tin liên hệ trực tiếp. Vui lòng sử
                      dụng form đặt phòng để được hỗ trợ.
                    </p>
                  )}
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                  {roomStats.availableCount} phòng trống · {rooms.length} phòng
                  đang vận hành
                </p>
              </div>
