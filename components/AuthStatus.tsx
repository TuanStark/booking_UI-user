...
export default function AuthStatus({ className = "" }: AuthStatusProps) {
  ...
  if (!isAuthenticated || !user) {
    return (
      <div className={cn("flex items-center space-x-3", className)}>
        ...
        <Link
          href="/auth/signup"
          className="bg-brand text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-brand-dark transition-all duration-200 shadow-lg hover:shadow-xl"
        >
          Đăng ký
        </Link>
      </div>
  ...
  const roleField = (user as { role?: string | { name?: string } } | undefined)
    ?.role;
  ...
  return (
    ...
    >
      ...
      >
        <span className="w-9 h-9 bg-brand rounded-full flex items-center justify-center shadow-md">
